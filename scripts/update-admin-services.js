...
/**
 * Script to update collection references in admin-services folder
 */
...
async function main() {
  console.log('🚀 Starting admin-services collection updates...');
  
  // Find all TypeScript files in admin-services
  const files = await glob('admin-services/**/*.ts', {
    cwd: process.cwd()
  });
  ...
  console.log('\n' + '='.repeat(50));
  console.log('📊 ADMIN-SERVICES UPDATE SUMMARY');
  console.log('='.repeat(50));
  console.log(`✅ Files updated: ${filesUpdated}`);
  console.log(`🔄 Total changes: ${totalChanges}`);
  console.log(`📋 Collections updated: ${Array.from(allCollections).join(', ')}`);
  console.log('\n🎉 Admin-services collection updates completed!');
}
