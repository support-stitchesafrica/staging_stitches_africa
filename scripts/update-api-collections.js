...
/**
 * Script to update collection references in app/api folder
 */
...
const COLLECTION_MAPPINGS = {
  ...
  'atlasUsers': 'staging_atlasUsers',
  'atlasInvitations': 'staging_atlasInvitations',
  'collectionsInvitations': 'staging_collectionsInvitations',
  'marketing_invitations': 'staging_marketing_invitations',
  'backoffice_users': 'staging_backoffice_users',
  'backoffice_invitations': 'staging_backoffice_invitations',
  'blog_admin_invitations': 'staging_blog_admin_invitations',
  'hierarchical_influencers': 'staging_hierarchical_influencers',
  'hierarchical_admins': 'staging_hierarchical_admins',
  'hierarchical_referrals': 'staging_hierarchical_referrals',
  'collection_waitlists': 'staging_collection_waitlists',
  'waitlist_subscriptions': 'staging_waitlist_subscriptions',
  'waitlist_analytics_events': 'staging_waitlist_analytics_events',
  'waitlist_analytics_counters': 'staging_waitlist_analytics_counters',
  'agent_chat_sessions': 'staging_agent_chat_sessions',
  'agent_chat_messages': 'staging_agent_chat_messages',
  'coupons': 'staging_coupons',
  'coupon_usage': 'staging_coupon_usage',
  'promotions': 'staging_promotions',
  'vouchers': 'staging_vouchers',
  'voucher_transactions': 'staging_voucher_transactions'
};
  ...
  try {
    ...
    for (const [oldCollection, newCollection] of Object.entries(COLLECTION_MAPPINGS)) {
      ...
      if (matches1) {
        ...
        if (!changedCollections.includes(oldCollection)) {
          changedCollections.push(oldCollection);
        }
      }
      ...
      // Pattern 5: .collection('collection_name') - for Firebase v8 style
      const pattern5 = new RegExp(`\\.collection\\(["']${oldCollection}["']\\)`, 'g');
      const replacement5 = `.collection("${newCollection}")`;
      const matches5 = updatedContent.match(pattern5);
      if (matches5) {
    ...
    if (changes > 0) {
      ...
      console.log(`✅ ${filePath}: ${changes} changes (${changedCollections.join(', ')})`);
      return { changes, collections: changedCollections };
    }

    return { changes: 0, collections: [] };
  } catch (error) {
...
async function main() {
  console.log('🚀 Starting API collection updates...');
  ...
  for (const file of files) {
    ...
    if (result.changes > 0) {
      ...
      filesUpdated++;
      result.collections.forEach(col => allCollections.add(col));
    }
  }

  console.log('\n' + '='.repeat(60));
  console.log('📊 API COLLECTION UPDATE SUMMARY');
  console.log('='.repeat(60));
  console.log(`✅ Files updated: ${filesUpdated}`);
  console.log(`🔄 Total changes: ${totalChanges}`);
  console.log(`📋 Collections updated: ${Array.from(allCollections).join(', ')}`);
  console.log('\n🎉 API collection updates completed!');
}
