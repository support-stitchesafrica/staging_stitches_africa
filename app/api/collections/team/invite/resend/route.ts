/**
 * Collections Team Invitation Resend API
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get the invitation
    const invitationDoc = await adminDb.collection('collectionsInvitations').doc(invitationId).get();
    ...
    const newInviteId = adminDb.collection('collectionsInvitations').doc().id;
    ...
    const JWT_SECRET = process.env.JWT_SECRET || 'collections-dashboard-secret';
    ...
    await adminDb.collection('collectionsInvitations').doc(newInviteId).set(newInvitation);

    // Revoke the old invitation
    await adminDb.collection('collectionsInvitations').doc(invitationId).update({
      status: 'revoked'
    });
