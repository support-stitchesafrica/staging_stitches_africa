...
const ShoesProductCard = React.memo(({ product, onWishlistToggle, isInWishlist }: {
	product: Product;
	...
	const { basePrice, discountedPrice } = useMemo(() => {
		...
		if (product.enableMultiplePricing && product.individualItems && product.individualItems.length > 0) {
			...
			base = Math.min(...product.individualItems.map((item: any) => item.price));
		} else {
			base = typeof product.price === 'number' ? product.price : product.price.base;
		}
		const discounted = product.discount > 0 ? base * (1 - product.discount / 100) : base;
		return { basePrice: base, discountedPrice: discounted };
	}, [product.price, product.discount, product.enableMultiplePricing, product.individualItems]);
...
ShoesProductCard.displayName = 'ShoesProductCard';
...
export default function ShoesPage() {
	...
	// Fetch shoes products with enhanced fallback logic
	const {
		data: shoesProductsRaw = [],
		loading: shoesLoading,
	} = useCachedData(
		'shoes-products',
		async () => {
			// First try category-specific search
			let products = await productRepository.getProductsByCategory('shoes');
			...
			if (products.length === 0) {
				...
				products = allProducts.filter(product => {
					...
					return title.includes('shoe') || title.includes('boot') || title.includes('sneaker') ||
						   ...
						   (product.category && product.category.toLowerCase().includes('shoe'));
				});
			}
	...
	const shoesProducts = Array.isArray(shoesProductsRaw) ? shoesProductsRaw : [];

	// Get subcategories from shoes products (based on tags or product titles)
	const subcategories = useMemo(() => {
		...
		shoesProducts.forEach(product => {
			...
			// Common shoe subcategories
			const shoeTypes = ['sneakers', 'boots', 'heels', 'flats', 'sandals', 'loafers', 'oxfords', 'pumps'];
			
			shoeTypes.forEach(type => {
				if (title.includes(type) || tags.some(tag => tag.toLowerCase().includes(type))) {
					subs.add(type.charAt(0).toUpperCase() + type.slice(1));
				}
			});
		});
	...
	const filteredProducts = useMemo(() => {
		...
		return filtered;
	}, [shoesProducts, selectedSubcategory, selectedType, selectedPriceRange, sortBy]);
		...
		<div className="min-h-screen bg-white">
			...
			<div className="border-b border-gray-200 py-4">
				<div className="container mx-auto px-4">
					<div className="flex items-center text-sm text-gray-600">
						...
						<span className="text-gray-900">Shoes</span>
					</div>
				</div>
			</div>
			...
			<div className="py-12 bg-gray-50">
				<div className="container mx-auto px-4 text-center">
					<h1 className="text-4xl font-light mb-4">Shoes</h1>
					<p className="text-gray-600 mb-8 max-w-2xl mx-auto">
						Step into style with our curated collection of footwear from African designers and international brands. 
						From everyday comfort to statement pieces, find your perfect pair.
					</p>
				</div>
			</div>
				...
				<div className="container mx-auto px-4">
					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<p className="text-sm text-gray-600">
							{filteredProducts.length} shoe {filteredProducts.length === 1 ? 'item' : 'items'}
