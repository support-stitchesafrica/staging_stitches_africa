...
const JewelryProductCard = React.memo(({ product, onWishlistToggle, isInWishlist }: {
	product: Product;
...
JewelryProductCard.displayName = 'JewelryProductCard';
...
export default function JewelryPage() {
	...
	// Fetch jewelry products with enhanced fallback logic
	const {
		data: jewelryProductsRaw = [],
		loading: jewelryLoading,
	} = useCachedData(
		'jewelry-products',
		async () => {
			// First try category-specific search
			let products = await productRepository.getProductsByCategory('jewelry');
			...
			if (products.length === 0) {
				...
				products = allProducts.filter(product => {
					...
					return title.includes('necklace') || title.includes('earring') || title.includes('bracelet') ||
						   ...
						   (product.category && product.category.toLowerCase().includes('jewelry'));
				});
			}
	...
	const jewelryProducts = Array.isArray(jewelryProductsRaw) ? jewelryProductsRaw : [];

	// Get subcategories from jewelry products (based on tags or product titles)
	const subcategories = useMemo(() => {
		...
		jewelryProducts.forEach(product => {
			...
			// Common jewelry subcategories
			const jewelryTypes = ['necklace', 'earring', 'bracelet', 'ring', 'pendant', 'chain', 'anklet', 'brooch'];
	...
	const filteredProducts = useMemo(() => {
		...
		return filtered;
	}, [jewelryProducts, selectedSubcategory, selectedType, selectedPriceRange, sortBy]);
		...
		<div className="min-h-screen bg-white">
			...
			<div className="py-12 bg-gray-50">
				<div className="container mx-auto px-4 text-center">
					<h1 className="text-4xl font-light mb-4">Jewelry</h1>
					<p className="text-gray-600 mb-8 max-w-2xl mx-auto">
						Shine bright with our curated collection of jewelry from African designers and international brands. 
						From everyday elegance to statement pieces, find your perfect sparkle.
					</p>
				</div>
			</div>
			...
			<div className="border-b border-gray-200 py-6">
				<div className="container mx-auto px-4">
					<div className="flex flex-wrap items-center gap-4 mb-4">
						...
						<FilterButton
							active={selectedPriceRange === '100-250'}
							onClick={() => setSelectedPriceRange(selectedPriceRange === '100-250' ? 'all' : '100-250')}
						>
							$100 - $250
						</FilterButton>
					</div>
					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<p className="text-sm text-gray-600">
							{filteredProducts.length} jewelry {filteredProducts.length === 1 ? 'item' : 'items'}
					...
					{jewelryLoading ? (
						...
						<div className="text-center py-12">
							<p className="text-gray-500 text-lg mb-4">
								No jewelry found matching your filters
							</p>
							<button
								onClick={() => {
