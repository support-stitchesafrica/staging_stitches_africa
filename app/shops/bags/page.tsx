...
const BagsProductCard = React.memo(({ product, onWishlistToggle, isInWishlist }: {
	product: Product;
...
BagsProductCard.displayName = 'BagsProductCard';
...
export default function BagsPage() {
	...
	// Fetch bags products with enhanced fallback logic
	const {
		data: bagsProductsRaw = [],
		loading: bagsLoading,
	} = useCachedData(
		'bags-products',
		async () => {
			try {
				...
				let products = await productRepository.getProductsByCategory('bags');
				console.log('Category search result:', products.length);
				
				// If no results, try broader search across all products
				if (products.length === 0) {
					console.log('No category results, trying broader search...');
					...
					products = allProducts.filter(product => {
						...
						const matches = title.includes('bag') || title.includes('purse') || title.includes('handbag') || 
							   ...
							   (product.category && product.category.toLowerCase().includes('bag'));
						
						if (matches) {
							console.log('Found bag product:', product.title);
						}
						return matches;
					});
	...
	const bagsProducts = Array.isArray(bagsProductsRaw) ? bagsProductsRaw : [];

	// Get subcategories from bags products (based on tags or product titles)
	const subcategories = useMemo(() => {
		...
		bagsProducts.forEach(product => {
			...
			// Common bag subcategories
			const bagTypes = ['handbag', 'backpack', 'tote', 'clutch', 'crossbody', 'shoulder', 'messenger', 'wallet', 'purse'];
	...
	const filteredProducts = useMemo(() => {
		...
		if (selectedPriceRange !== 'all') {
			filtered = filtered.filter(product => {
				...
				if (product.enableMultiplePricing && product.individualItems && product.individualItems.length > 0) {
					...
					price = Math.min(...product.individualItems.map((item: any) => item.price));
				} else {
					price = typeof product.price === 'number' ? product.price : product.price.base;
				}
		...
		filtered.sort((a, b) => {
			switch (sortBy) {
				...
				case 'price-asc':
					// Determine base price - use minimum individual item price if multiple pricing is enabled
					let priceA: number;
					if (a.enableMultiplePricing && a.individualItems && a.individualItems.length > 0) {
						priceA = Math.min(...a.individualItems.map((item: any) => item.price));
					} else {
						priceA = typeof a.price === 'number' ? a.price : a.price.base;
					}
					let priceB: number;
					if (b.enableMultiplePricing && b.individualItems && b.individualItems.length > 0) {
						priceB = Math.min(...b.individualItems.map((item: any) => item.price));
					} else {
						priceB = typeof b.price === 'number' ? b.price : b.price.base;
					}
				...
				case 'price-desc':
					...
					let priceA2: number;
					if (a.enableMultiplePricing && a.individualItems && a.individualItems.length > 0) {
						priceA2 = Math.min(...a.individualItems.map((item: any) => item.price));
					} else {
						priceA2 = typeof a.price === 'number' ? a.price : a.price.base;
					}
					let priceB2: number;
					if (b.enableMultiplePricing && b.individualItems && b.individualItems.length > 0) {
						priceB2 = Math.min(...b.individualItems.map((item: any) => item.price));
					} else {
						priceB2 = typeof b.price === 'number' ? b.price : b.price.base;
					}
		...
		return filtered;
	}, [bagsProducts, selectedSubcategory, selectedType, selectedPriceRange, sortBy]);
		...
		<div className="min-h-screen bg-white">
			...
			<div className="py-12 bg-gray-50">
				<div className="container mx-auto px-4 text-center">
					<h1 className="text-4xl font-light mb-4">Bags</h1>
					<p className="text-gray-600 mb-8 max-w-2xl mx-auto">
						Carry your style with our curated collection of bags from African designers and international brands. 
						From everyday essentials to statement pieces, find your perfect companion.
					</p>
				</div>
			</div>
				...
				<div className="container mx-auto px-4">
					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<p className="text-sm text-gray-600">
							{filteredProducts.length} bag {filteredProducts.length === 1 ? 'item' : 'items'}
					...
					{bagsLoading ? (
						...
						<div className="text-center py-12">
							<p className="text-gray-500 text-lg mb-4">
								No bags found matching your filters
							</p>
							<button
								onClick={() => {
