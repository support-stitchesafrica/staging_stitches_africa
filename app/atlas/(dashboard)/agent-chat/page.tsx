...
export default function AgentChatPage() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (sessions.length === 0) {
        ...
        // Retry after 3 seconds in case the user document is being created
  ...
  const handleSessionSelect = async (session: AgentChatSession) => {
    ...
    if (session.status === 'pending' && atlasUser) {
      try {
  ...
  const handleSendMessage = async () => {
    if (!selectedSession || !newMessage.trim() || !atlasUser) return;
    ...
    try {
      await agentChatService.addMessage(selectedSession.id, {
        ...
        messageType: 'text',
        agentId: atlasUser.uid,
        agentName: atlasUser.fullName
      });
  ...
  const handleCloseSession = async (sessionId: string) => {
    ...
    try {
      await agentChatService.closeSession(sessionId, atlasUser.uid);
