...
import { getFirestore, collection, collectionGroup, query, where, getDocs, doc, updateDoc, writeBatch } from "firebase/firestore";
...
export function OrdersTab({ orders, tailorData }: OrdersTabProps)
{
	...
	const handleStatusUpdate = async (order: OrderRow, newStatus: string) =>
	{
		try
		{
			...
			// The shared cart reference stored on the document (not the Firestore doc ID)
			const sharedOrderId: string | undefined = (order.raw as any).order_id;

			// Find all sibling item documents that share the same order_id
			const getSiblingDocs = async (): Promise<{ userId: string; docId: string }[]> =>
			{
			...
			// For DHL callables: pass the full doc id (VVIP uses "{docId}_{itemIndex}")
			const dhlOrderId = orderId;

			const willRunDhlGeneration =
				newStatus === "Ready for shipment" &&
				!order.raw.dhl_shipment?.shipmentTrackingNumber;

			// If moving to "Ready for shipment", create DHL shipment first (skip if label already exists)
			if (willRunDhlGeneration)
			{
				const result = await generateDhlShipmentForOrder(userId, dhlOrderId);
				if (!result?.ok)
				{
				...
				const siblings = await getSiblingDocs();
				...
				if (siblings.length > 1)
				{
					...
					{
						const ref = doc(db, "users_orders", sib.userId, "user_orders", sib.docId);
						batch.update(ref, {
							dhl_shipment: result.shipment,
					...
					setOrderData((prev) =>
						prev.map((o) =>
						{
							const isInGroup = siblings.some((s) => s.docId === o.id);
							return isInGroup ? { ...o, dhl_shipment: uiShipment, order_status: newStatus } : o;
						})
					);
					toast.success(`📦 DHL shipment created and applied to all ${siblings.length} items in this order`);
				} else
				{
					// Single line item: shipment shown immediately; `order_status` is written in persist below.
					setOrderData((prev) =>
						prev.map((o) =>
			...
			// Persist order_status. "Ready for shipment" used to be skipped here, so the backend
			// never got order_status for single-line items (multi-line was set in the DHL batch above).
			const siblings = await getSiblingDocs();
			const readyStatusAlreadyInDhlBatch = willRunDhlGeneration && siblings.length > 1;

			if (newStatus === "Ready for shipment" && !readyStatusAlreadyInDhlBatch)
			{
				if (siblings.length > 1)
				{
					const db = getFirestore(app);
					...
					{
						const ref = doc(db, "users_orders", sib.userId, "user_orders", sib.docId);
						batch.update(ref, { order_status: newStatus });
					}
					await batch.commit();
					setOrderData((prev) =>
						prev.map((o) =>
						{
							const isInGroup = siblings.some((s) => s.docId === o.id);
							return isInGroup ? { ...o, order_status: newStatus } : o;
						})
					);
				} else
				{
			...
			} else if (newStatus !== "Ready for shipment" && siblings.length > 1)
			{
				const db = getFirestore(app);
				const batch = writeBatch(db);
				for (const sib of siblings)
			...
			} else if (newStatus !== "Ready for shipment")
			{
				await updateOrderStatus(userId, orderId, newStatus);
				setOrderData((prev) =>
					prev.map((o) =>
	...
	async function scheduleDhlPickup(order: OrderRow)
	{
		try
		{
			...
			console.log("📦 DHL Pickup payload:", {
				...
				locationType: order.raw.dhl_shipment?.type === "domestic" ? pickupForm.locationType : undefined,
			});
		...
		} catch (err: any)
		{
			...
			// Firebase INTERNAL errors often have a details field with the real message
			const detail = err?.details ?? err?.message ?? "❌ Failed to schedule pickup";
			console.error("Pickup error detail:", detail, "| code:", err?.code);
																...
																<div className="flex space-x-2 overflow-x-auto">
																	{selectedOrder.raw.images.map((img, idx) => (
																		<img
																			...
																			src={img}
																			alt={`Thumbnail ${idx + 1}`}
																			className={`w-16 h-16 object-cover rounded-md border cursor-pointer ${previewImage === img
																				? "border-blue-500"
																				: "border-gray-200"
																				}`}
  ...
  `}
														...
														{/* Re-trigger mirror for VVIP orders (backfills user_orders so DHL can find the order) */}
														{selectedOrder && (
															(selectedOrder.raw as any).isVvip === true ||
															selectedOrder.raw.delivery_type === "manual_transfer" ||
															/^.+_\d+$/.test(selectedOrder.raw.id)
														) && (
																<div className="mt-3">
																	<Button
																		variant="outline"
																		...
																		onClick={async () =>
																		{
																			const vvipMatch = selectedOrder.raw.id.match(/^(.+)_(\d+)$/);
																			const realId = vvipMatch ? vvipMatch[1] : selectedOrder.raw.id;
																			const tailorId = tailorData?.id || selectedOrder.raw.tailor_id;
																			try
																			{
																				const res = await fetch("/api/admin/vvip-orders/mirror", {
																					method: "POST",
																					headers: { "Content-Type": "application/json" },
																					body: JSON.stringify({ orderId: realId, tailorId }),
																				});
																				const data = await res.json();
																				if (res.ok)
																				{
																					toast.success(`✅ Order re-triggered: ${data.message}`);
																				} else
																				{
