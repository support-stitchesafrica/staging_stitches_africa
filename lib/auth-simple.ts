...
export const signInWithEmail = async (email: string, password: string): Promise<AuthResult> => {
  try {
    ...
    return { user: result.user, error: null, isRetryable: false };
  } catch (error: any) {
    return { user: null, error: getAuthErrorMessage(error.code || 'unknown'), isRetryable: getIsRetryableAuthError(error.code || 'unknown') };
  }
};
...
function getIsRetryableAuthError(errorCode: string): boolean {
  return ['auth/network-request-failed','auth/timeout','auth/internal-error','auth/cancelled-popup-request','auth/popup-blocked','auth/popup-closed-by-user'].includes(errorCode);
}
