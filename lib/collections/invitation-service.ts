/**
 * Collections Dashboard Invitation Service
 * Handles invitation creation, validation, and acceptance workflow
 * Following the Marketing dashboard pattern
 */
...
const COLLECTIONS = {
  INVITATIONS: 'staging_collectionsInvitations'
} as const;
 ...
 * Collections Dashboard Invitation Service
 */
export class CollectionsInvitationService {
  /**
   ...
   */
  static async createInvitation(data: CreateCollectionsInvitationData): Promise<CollectionsInvitation> {
    // Normalize email
    ...
    const invitation: CollectionsInvitation = {
      id: inviteId,
      email: normalizedEmail,
      name: data.name,
      role: data.role,
  ...
  static async validateInvitation(token: string): Promise<InvitationValidationResult> {
    try {
      ...
      const validation = validateInvitationToken(decodedToken, 'collections');
      ...
      const invitation = { id: inviteDoc.id, ...inviteDoc.data() } as CollectionsInvitation;
      ...
      console.error('[Collections Invitation Service] Validation error:', error);
      return {
        isValid: false,
   ...
   */
  static async acceptInvitation(token: string, acceptedByUid?: string): Promise<CollectionsInvitation> {
   ...
   */
  static async getInvitationByEmail(email: string): Promise<CollectionsInvitation | null> {
    ...
    return { id: doc.id, ...doc.data() } as CollectionsInvitation;
  }

  /**
   * Get invitation by ID
   */
  static async getInvitationById(id: string): Promise<CollectionsInvitation | null> {
    ...
    return { id: inviteDoc.id, ...inviteDoc.data() } as CollectionsInvitation;
  }
   ...
   * Get all invitations
   */
  static async getAllInvitations(): Promise<CollectionsInvitation[]> {
    const querySnapshot = await getDocs(collection(db, COLLECTIONS.INVITATIONS));
    
    return querySnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    })) as CollectionsInvitation[];
  }

  /**
   * Get invitations by status
   */
  static async getInvitationsByStatus(status: CollectionsInvitation['status']): Promise<CollectionsInvitation[]> {
    const q = query(
      collection(db, COLLECTIONS.INVITATIONS),
      where('status', '==', status)
    );
   ...
   */
  static async getInvitationsByInviter(invitedByUserId: string): Promise<CollectionsInvitation[]> {
    const q = query(
      collection(db, COLLECTIONS.INVITATIONS),
  ...
  static generateInvitationLink(token: string, baseUrl?: string): string {
    ...
    return `${base}/collections/invite/${token}`;
  }
...
export const collectionsInvitationUtils = {
  generateInvitationLink: CollectionsInvitationService.generateInvitationLink,
  canCreateInvitation: CollectionsInvitationService.canCreateInvitation,
  canManageInvitations: CollectionsInvitationService.canManageInvitations,
  checkEmailExists: CollectionsInvitationService.checkEmailExists
};

// Export error types for better error handling
export class CollectionsInvitationError extends Error {
  constructor(
    message: string,
    public code: string,
    ...
    super(message);
    this.name = 'CollectionsInvitationError';
  }
}

export const CollectionsInvitationErrorCodes = {
